...
class MultiLanguage3 extends Migration
{
    /**
     * Run the migrations.
     *
            ...
            BEGIN
              ...
              LOOP
                IF NEW.attrs->param.slug IS NULL THEN
                  RAISE EXCEPTION 'Параметр % является обязательным!', param.title;
                END IF;
                ...
                IF param.unique = true THEN
                  ...
                  IF tmp > 0 THEN
                    RAISE EXCEPTION 'Параметр % должен иметь уникальное значение!', param.title;
                  END IF;
                END IF;
              END LOOP;
